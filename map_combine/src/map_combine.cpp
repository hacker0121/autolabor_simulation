...
void match_callback(const std_msgs::String& m)
{
    m_status = m.data;
}
...
main (int argc, char **argv)
{
    ...
    matching_sub = nh.subscribe("match", 100, match_callback);
    point_pub = nh.advertise<sensor_msgs::PointCloud2>("points_map", 100);
    client = nh.serviceClient<map_load::load>("map_load");

    nh.param<float>("grid_distant", dis, 7.0);
    nh.param<std::string>("dir", dir, "/home/eric/Desktop/map2/%s");
    ...
    while (nh.ok())
    {
        ...
        if (m_status == "not_match")
        {
            sprintf(cstr[0], dir.c_str(), (floors + "/" +str[0]).c_str());
            pcl::io::loadPCDFile(cstr[0],cloud_n);
            pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);
            *cloud_ptr = cloud_n;
            voxel.setInputCloud(cloud_ptr);
            voxel.setLeafSize(0.4, 0.4, 0.4);
            voxel.filter(filter_cloud[0]);
            cloud = filter_cloud[0];
        }
        else if (m_status == "match")
        {
            for (int i = 0; i < 9; i++)
            {
                sprintf(cstr[i], dir.c_str(), (floors + "/" +str[i]).c_str());
